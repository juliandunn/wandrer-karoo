...
rm -rf platform-tools/ rm platformtools.zip > /dev/null
rm devices > /dev/null
...
platformtools_url=https://dl.google.com/android/repository/platform-tools_r${platformtools_version}-darwin.zip
printf "\nDownloading... \n  ${platformtools_url}\n"
curl --show-error  --progress-bar -o  platformtools.zip ${platformtools_url}

# unzip platform tools
unzip platformtools.zip > /dev/null

# confirm adb version
printf "\nChecking ADB:\n"
# pipe to sed to add two spaces to each output line
./platform-tools/adb --version | sed 's/^/  /'
...
mfg=$(./platform-tools/adb shell getprop | grep ro.product.manufacturer)
model=$(./platform-tools/adb shell getprop | grep ro.product.model)
...
if [[ $mfg == *"Hammerhead"* ]] ; then
 ...
 if [[ "$model" == *"k2"* || "$model" == *"Karoo"* ]] ; then
    ...
    ./platform-tools/adb install -r ${APK} > /dev/null
    
    # print newly installed version name
    version=$(./platform-tools/adb shell dumpsys package ${wandrer_package} \
      | grep versionName \
      | sed 's/versionName=//')
    printf "  Installed version: ${version}\n  Rebooting...\n"
    ./platform-tools/adb shell pm clear ${wandrer_package} > /dev/null
    ./platform-tools/adb reboot > /dev/null
    sleep 50 # seconds, just a guess
    ./platform-tools/adb shell am start -n ${wandrer_package}/${starting_activity} > /dev/null
